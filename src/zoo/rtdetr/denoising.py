...
def get_contrastive_denoising_training_group(
    ...
    if box_noise_scale > 0:
        ...
        if use_scale_aware_dn:

            gt_area = input_query_bbox[..., 2] * input_query_bbox[..., 3]

            scale_factor = (gt_area / sadt_area_threshold).clamp(
                min=sadt_min_factor, max=1.0
            )

            diff = diff * scale_factor.unsqueeze(-1)
