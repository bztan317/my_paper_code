...
class RTDETRTransformerv2(nn.Module):
    ...
    def __init__(
        ...
        use_scale_balanced_query=False,
        sbqs_min_ratio=0.15,
        use_scale_aware_dn=False,
        sadt_area_threshold=0.01,
        sadt_min_factor=0.3,
    ):
        super().__init__()
        ...
        self.use_scale_balanced_query = use_scale_balanced_query
        self.sbqs_min_ratio = sbqs_min_ratio
        ...
        self.use_scale_aware_dn = use_scale_aware_dn
        self.sadt_area_threshold = sadt_area_threshold
        self.sadt_min_factor = sadt_min_factor
        if num_denoising > 0:
    ...
    def _get_decoder_input(
        ...
        if self.use_scale_balanced_query:
            enc_topk_memory, enc_topk_logits, enc_topk_bbox_unact = (
                self._select_topk_balanced(
                    output_memory,
                    enc_outputs_logits,
    ...
    def _select_topk_balanced(
        ...
        min_per_level = max(1, int(topk * self.sbqs_min_ratio))

        if min_per_level * num_levels > topk:
            min_per_level = topk // num_levels

        guaranteed_parts = []
        for lvl in range(num_levels):
            start_idx = level_starts[lvl]
            level_sz = level_sizes[lvl]
            level_scores = scores[:, start_idx : start_idx + level_sz]
            k = min(min_per_level, level_sz)
            _, local_topk = torch.topk(level_scores, k, dim=-1)
            guaranteed_parts.append(local_topk + start_idx)

        guaranteed = torch.cat(guaranteed_parts, dim=-1)

        remaining = topk - guaranteed.shape[1]
        if remaining > 0:
            B, N = scores.shape
            mask = torch.zeros(B, N, device=scores.device, dtype=torch.bool)
            mask.scatter_(1, guaranteed, True)
            masked_scores = scores.clone()
            masked_scores[mask] = float("-inf")
            _, global_topk = torch.topk(masked_scores, remaining, dim=-1)
            topk_ind = torch.cat([guaranteed, global_topk], dim=-1)
        else:
            topk_ind = guaranteed[:, :topk]
        ...
        if self.training and self.num_denoising > 0:
            denoising_logits, denoising_bbox_unact, attn_mask, dn_meta = (
                get_contrastive_denoising_training_group(
                    ...
                    use_scale_aware_dn=self.use_scale_aware_dn,
                    sadt_area_threshold=self.sadt_area_threshold,
                    sadt_min_factor=self.sadt_min_factor,
                )
            )
        else:
            denoising_logits, denoising_bbox_unact, attn_mask, dn_meta = (
