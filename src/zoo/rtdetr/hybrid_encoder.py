...
class HybridEncoder(nn.Module):
    ...
    def forward(self, feats, stage0_feat=None, img=None):
        assert len(feats) == len(self.in_channels)
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]

        if hasattr(self, "ega_neck") and stage0_feat is not None:
            ega_outputs = self.ega_neck(stage0_feat, *proj_feats)
            for i, feat in enumerate(ega_outputs):
                proj_feats[i] = feat
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            fused = torch.concat([upsample_feat, feat_low], dim=1)
            inner_out = self.fpn_blocks[fpn_idx](fused)
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            fused = torch.concat([downsample_feat, feat_height], dim=1)
